...
double get_nom(double *current, int k, int sign) {
    double nom = 0;
    for (int i_v = -V; i_v < V + 1; i_v++) {
        double v = i_v * dv;
        if (sign * v > 0) {
    ...
    for (int i_vx = -V_X; i_vx < V_X + 1; i_vx++) {
        for (int i_vy = -V_Y; i_vy < V_Y + 1; i_vy++) {
            ...
            if (v > 0) {
                for (int k_x = 0; k_x < K_X; k_x++) {
                    for (int k_y = 0; k_y < K_Y; k_y++) {
                        if ((k_y == 0 and k_x < wall_x_left) or (k_x > wall_x_left and k_x < wall_x_right and k_y == wall_top_y)) {
                            next[index(k_x, k_y, i_vx, i_vy)] = next[index(k_x, k_y, i_vx, -i_vy)];
                        } else if (k_y == 0 and k_x > wall_x_right) {
                            next[index(k_x, k_y, i_vx, i_vy)] = 0;
                        } else {
                ...
                for (int k_x = 0; k_x < K_X; k_x++) {
                    for (int k_y = 0; k_y < K_Y; k_y++) {
                        if ((k_y == K_Y - 1 and k_x < wall_x_left) or (k_x > wall_x_left and k_x < wall_x_right and k_y == wall_bottom_y)) {
                            next[index(k_x, k_y, i_vx, i_vy)] = next[index(k_x, k_y, i_vx, -i_vy)];
                        } else if (k_y == K_Y - 1 and k_x > wall_x_right) {
                            next[index(k_x, k_y, i_vx, i_vy)] = 0;
                        } else {
...
void save_to_file_kin(double* data, int i) {
    ...
    for (int k_x = 0; k_x < K_X; k_x++) {
        for (int k_y = 0; k_y < K_Y; k_y++) {
            ...
            for (int i_vx = -V_X; i_vx < V_X + 1; i_vx++) {
                for (int i_vy = -V_Y; i_vy < V_Y + 1; i_vy++) {
                    pres += data[index(k_x, k_y, i_vx, i_vy)]*((i_vx*dvx)*(i_vx*dvx) + (i_vy*dvy)*(i_vy*dvy))/2;
                }
            }
            file << k_x << " " << k_y << " " << concetration << std::endl;
        }
